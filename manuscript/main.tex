...
\begin{abstract}
The 0/1 knapsack problem is a canonical NP-hard challenge serving as a fundamental model for resource allocation. While existing literature emphasizes predicting empirical hardness from static, a priori instance features alone, practical deployment often requires online performance prediction---estimating remaining runtime or search-tree size after observing the initial execution trajectory. This study evaluates how predictive information accumulates during algorithm execution for Branch and Bound (B\&B), Dynamic Programming (DP), and Greedy heuristics across 6,000 instances spanning five Pisinger structural families. We methodically separate baseline predictability (using 42 static features) from early-execution predictability (incorporating search dynamics snapped at 1\%, 5\%, 10\%, and 25\% of the execution budget). While full-sample static predictability is high for deterministic Greedy and DP runtimes ($R^2_{adj} \ge 0.856$), B\&B search trees require early-execution metrics (e.g., bound-gap variance) to achieve comparable in-sample accuracy, suggesting that predictive information may accumulate within the first 5\% of search. However, under Leave-One-Family-Out (LOFO) cross-validation, models that appear near-perfect in-sample ($R^2_{adj} \approx 0.98$) substantially fail on held-out structurally extreme families (producing worse-than-random predictions, $R^2 \ll 0$). This reveals a marked in-sample/out-of-distribution gap, indicating that while early-execution metrics are highly informative within a structural regime, runtime predictability appears constrained, within this benchmark, by a structural generalization boundary.
\end{abstract}
...
The 0/1 knapsack problem is a canonical NP-hard combinatorial optimization challenge, serving as a fundamental model for resource allocation. Understanding and predicting the empirical performance of algorithms designed to solve these problems is critical for operational deployment in latency-sensitive environments, such as cloud resource scheduling and algorithm portfolios. Prior research has established a strong relationship between static, a priori instance characteristics and the expected computational behavior of algorithms. However, purely static prediction ignores the wealth of dynamic information generated once an algorithm begins executing. In practice, observing the first fraction of a solver's search trajectory often provides critical clues about the remaining runtime---enabling early-stopping heuristics, dynamic timeout allocation, and mid-flight algorithm switching.

This study investigates the online predictability of algorithm performance, evaluating how quickly predictive information accumulates during search and whether this information transfers across structurally diverse problem topologies. We address the following research questions:

\begin{itemize}
  \item \textbf{RQ1 (A Priori Predictability):} What is the baseline predictability of algorithmic performance using only static, pre-execution instance characteristics?
  \item \textbf{RQ2 (Online Information Accumulation):} How does predictive accuracy evolve during execution? Specifically, how much incremental variance ($\Delta R^2$) is explained by incorporating early-execution metrics captured at fractional search budgets (e.g., 1\%, 5\%, 25\%)?
  \item \textbf{RQ3 (Structural Generalization):} Does the predictive signal---both static and early-execution---transfer out-of-distribution across distinct structural families, or are these models constrained by a structural generalization boundary?
\end{itemize}

To address these questions, this study evaluates the online predictability of Branch and Bound (alongside Greedy and Dynamic Programming baselines). It finds that while static predictability is high for deterministic baselines, Branch and Bound performance is substantially better predicted once the initial search trajectory is observed ($\Delta R^2_{adj} \ge 0.217$). However, under Leave-One-Family-Out evaluation, predictive performance drops substantially ($R^2 < 0$ on several family holdouts), revealing a structural generalization boundary that separates interpolative from extrapolative predictive power.
...
\subsection{Positioning This Study: A Structured Gap Analysis}
While prior work has thoroughly investigated static algorithm selection and empirical hardness, three critical gaps remain:
\begin{enumerate}
    \item \textbf{Online vs. A Priori Predictability:} Existing literature focuses predominantly on predicting performance \emph{before} execution. There is a lack of systematic evaluation regarding how rapidly predictive information accumulates once the solver is initialized.
    \item \textbf{Generalization Across Topologies:} Most benchmark studies rely on uniform cross-validation, which assumes training and test sets are drawn from the same generative distribution. The extent to which predictive models can extrapolate to fundamentally different structural topologies remains underexplored.
    \item \textbf{Solver Sensitivity:} Hardness is often treated as an inherent property of the instance, rather than a joint property of the instance and the specific solver implementation (e.g., Best-Bound vs. Depth-First search).
\end{enumerate}

This study explicitly addresses these gaps by separating static from dynamic predictive contributions, testing strictly out-of-distribution generalization, and evaluating solver-implementation sensitivity.
\section{Methodology}
...
\subsection{A Priori Predictability from Static Features (RQ1 - Confirmatory)}\label{sec:rq1}

RQ1 asks what baseline predictability is achievable using only static, pre-execution instance characteristics. The five Pisinger families differ substantially in their weight--value correlation structure, and this structural variation is associated with systematic differences in algorithm performance. In \texttt{StronglyCorrelated} instances ($v_i = w_i + c$), profit/weight efficiency ratios are nearly uniform, depriving both Greedy sorting and B\&B bounding of discriminating signal; these instances produce the highest median B\&B node counts in the dataset. In \texttt{InverseCorrelated} instances ($v_i = W - w_i$), the ratio-based Greedy sort coincides with the optimal weight-ascending order, making Greedy empirically optimal on this family within this benchmark (zero optimality gap across all 1,200 instances). In \texttt{AlmostEqualRatios} instances, near-identical efficiency ratios produce degenerate B\&B bounds that fail to prune; median nodes explored are substantially higher than any other family at matched $n$ (see Table~\ref{tab:ols_metrics}).

Among the 42 instance characteristics, permutation importance analysis identifies \texttt{slack} ($\sum w_i - W$) and \texttt{total\_weight} as the strongest cross-family predictors of B\&B node exploration (largest permutation importance drops in held-out $R^2$), reflecting the capacity tightness mechanism: tighter capacity constrains feasible sets and focuses the search. Correlation metrics (\texttt{pearson\_corr}, \texttt{spearman\_corr}) and ratio uniqueness features (\texttt{unique\_ratio\_count}, \texttt{duplicate\_ratio\_fraction}) capture the structural differentiation between families. These associations address RQ1: instance characteristics carry strong, family-structured associations with algorithmic hardness, and this structure accounts for most in-sample predictive performance within this benchmark.
...
\subsection{Online Information Accumulation (RQ2 - Exploratory)}\label{sec:rq2}
\begin{itemize}
  \item \textbf{Objective:} To address RQ2, we evaluated the baseline predictability (M1) and augmented models capturing early-execution metrics (M2).
...
\subsection{Structural Generalization Boundary (RQ3 - Exploratory)}\label{sec:robustness}
\begin{itemize}
  \item \textbf{Objective:} To address RQ3, we subjected the models to four diagnostic tests: multicollinearity thinning (VIF), formal heteroskedasticity testing (Breusch-Pagan), regularization stability (Elastic-Net path analysis), and LOFO out-of-distribution evaluation.
  ...
  \item \textbf{Formal Heteroskedasticity (Breusch-Pagan):} All five OLS models exhibit formally confirmed heteroskedasticity at $p < 10^{-4}$: Greedy log\_time (BP = 170.7, $p < 10^{-16}$), DP log\_time (BP = 328.0, $p < 10^{-41}$), DP log\_memory (BP = 98.6, $p < 10^{-4}$), B\&B log\_time (BP = 335.4, $p < 10^{-37}$), B\&B log\_nodes (BP = 347.6, $p < 10^{-40}$). This suggests OLS standard errors may not accurately capture estimation uncertainty in the tails. The $R^2$-based incremental variance conclusions are unaffected by heteroskedasticity, but coefficient-level inference (standard errors, $t$-statistics) should be treated as approximate.
  ...
  \item \textbf{LOFO Generalization Failure:} Despite in-sample and 5-fold stability, LOFO evaluation reveals substantial generalization failures ($R^2 < 0$ on multiple outcome-family combinations). Predicting B\&B \texttt{log\_nodes\_explored} (M2) on the held-out \texttt{Uncorrelated} family results in $R^2 = -5.777$ (M1: 0.538); on \texttt{AlmostEqualRatios}, B\&B \texttt{log\_time\_millis} (M2) achieves $R^2 = -4.145$ (M1: 0.397). DP \texttt{log\_memory\_mb} (LOFO M1: $-107.6$; M2: $-110.4$) and Greedy \texttt{log\_time\_millis} (LOFO M1: $-4.66$; M2: $-5.07$) also demonstrate worse-than-random out-of-distribution prediction.\footnote{Note: Table~\ref{tab:lofo_shape} reports M2 $R^2 = -0.109$ for Uncorrelated on B\&B \texttt{log\_nodes\_explored}. That table uses a re-fitted M2 specification from the shape-analysis (M3) experiment, which employs a slightly different training set. The value $-5.777$ reported here is from the primary LOFO pipeline using the full M2 feature set.} Crucially, 5-fold CV does not reveal this failure (5-fold B\&B log\_nodes M2 $R^2 = 0.978$), consistent with the failure mechanism being structural distribution shift across families rather than standard overfitting.
  ...
  \item \textbf{Conclusion:} In-sample predictive associations and 5-fold stability are both strong. LOFO evaluation reveals substantial structural generalization failure across 8 of 15 evaluated outcome-family combinations ($R^2 < 0$). The generalization boundary --- defined by which family-holdout combinations remain partially predictive vs.\ which collapse --- constitutes the paper's central empirical finding.
\end{itemize}
...
\subsection{Solver Sensitivity and Hardness Bounds (Exploratory)}\label{sec:solver_sensitivity}

\subsubsection{Solver Implementation Sensitivity}
To evaluate whether empirical hardness is an invariant property of the instance or fundamentally tied to the solver implementation, we constructed a depth-first search (DFS) variant of Branch and Bound. We compared its node exploration to the standard best-bound-first (BBS) implementation on 100 benchmark instances ($n \in \{50, 100\}$). While BBS explores fewer nodes on average, DFS occasionally explores fewer nodes on specific, highly-correlated instances by rapidly identifying a deep incumbent that prunes the rest of the search tree. These results suggest solver-implementation sensitivity on this small sample: predictive models trained on one variant may not apply to another without validation, even when both solve the same mathematical formulation.

\subsubsection{Zero-Event Formalization}
The structural benchmark produces a notable class imbalance: certain structural families (\texttt{Uncorrelated}, \texttt{WeaklyCorrelated}) are solved optimally 100\% of the time within the node limit, creating a statistical zero-event problem for predictive models estimating the probability of timeout. We fit a logistic regression model with L1 penalization to estimate $P(\text{timeout})$ directly from structural features. The results indicate that the number of duplicate items (\texttt{duplicate\_pairs}) and the capacity tightness (\texttt{average\_fillable\_items}) are the strongest predictors of observing a timeout, providing empirical evidence of a link between instance structure and worst-case empirical hardness in this benchmark.

\subsection{Distributional Shape as Supplementary Execution Features (Exploratory)}\label{sec:rq3_old}
...
The most important limitation is the marked gap between in-sample and LOFO out-of-distribution performance. Models that achieve $R^2_{adj} \approx 0.98$ in-sample collapse to $R^2 < 0$ under LOFO for structurally extreme families. This failure is not overfitting in the standard sense (5-fold CV confirms near-identical performance to full-sample), but rather structural distribution shift: the weight--value correlation topology of \texttt{AlmostEqualRatios} and \texttt{InverseCorrelated} instances is qualitatively different from moderate families, and the execution metric instrumentation tested here (M2, M3) does not rescue generalization to these families. Any practical application of these models for algorithm selection or runtime prediction should be restricted to instances structurally similar to the training distribution.
...
All benchmark experiments were conducted using a Java~17 implementation of the three 0/1 knapsack solvers (Greedy, Dynamic Programming, and Branch and Bound). The Java runtime was OpenJDK~17.0, and Apache Commons CSV~1.10.0 was used for structured output. All statistical analyses were performed using Python~3 with the following library versions: NumPy~1.26, SciPy~1.11, pandas~2.1, statsmodels~0.14, and scikit-learn~1.3. Figures were rendered with Matplotlib~3.8. The manuscript was compiled using the Tectonic~0.14 LaTeX engine.

All experiments were executed on a single workstation. Runtime measurements (\texttt{time\_millis}) are hardware-dependent and may differ on other systems; the hardware-agnostic \texttt{log\_nodes\_explored} outcome is the primary recommended metric for cross-platform comparison.

\subsection{Dataset Generation}
All 6,000 knapsack instances were generated using Pisinger's~\cite{Pisinger2005} parametric methodology with random seed~42. The five structural families (\texttt{Uncorrelated}, \texttt{WeaklyCorrelated}, \texttt{StronglyCorrelated}, \texttt{InverseCorrelated}, \texttt{AlmostEqualRatios}) each contain 1,200 instances, equally split between fixed-capacity ($W = 1{,}000$) and scaled-capacity ($W = 0.5 \times \sum w_i$) modes. Item counts $n$ were drawn from the set $\{20, 50, 100, 200, 500, 1{,}000\}$, with 100 independently seeded instances per $(n, \text{family}, \text{capacity mode})$ combination. The generation pipeline is implemented in \texttt{src/main/java/dataset/} and is fully deterministic given the fixed seed.

\subsection{Reproduction Instructions}
The complete pipeline --- from instance generation through PDF compilation --- is encapsulated in a single shell script:
\begin{verbatim}
./reproduce.sh
\end{verbatim}
This script (1) compiles and runs the Java benchmark, (2) combines fixed- and scaled-capacity results, (3) generates all EDA figures and tables, (4) builds the regression LaTeX tables from the statistical modeling outputs, and (5) recompiles this manuscript. The expected wall-clock time for a full run is approximately 4--8~hours depending on hardware. An abbreviated smoke test using two instance sizes and five seeds per configuration can be executed in under one minute via \texttt{./build\_and\_run.sh 20,50 1000 5 42}.
