...
All three algorithms operate within a narrow memory band at fixed W=1000. Median heap usage is 0.50 MB for DP across all sizes, 0.50 MB for B&B across most sizes, and 0.50 MB for Greedy at all sizes.

B&B shows the only notable outlier behavior: maximum observed memory is 77 MB on Inverse Correlated instances at n=1000, where the search tree explores up to 50M nodes and the priority queue grows proportionally. On all other families, B&B memory stays at 0.50 MB even at n=1000.
