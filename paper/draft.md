...
B&B shows the only notable outlier behavior: maximum observed memory is 96 MB on Inverse Correlated instances, where the search tree explores up to 50M nodes and the priority queue grows proportionally. On all other families, B&B memory stays at 1.00 MB even at n=1000.
...
**DP: theoretical O(nW), observed O(n).** With W=1000 fixed, the theoretical complexity predicts linear scaling in n. The observed mean times (n=20: 0.02 ms, n=1000: 0.97 ms) yield an empirical ratio of 48.7x for a 50x increase in n, corresponding to a fitted exponent of approximately 0.99 on a log-log plot. The near-unit exponent confirms that DP's runtime is dominated by the O(nW) loop with negligible constant overhead at these scales. This is consistent with O(n) scaling at fixed W, confirming the theoretical prediction.

**Greedy: theoretical O(n log n), observed approximately O(n).** Mean times scale from 0.01 ms (n=20) to 0.16 ms (n=1000), a 14.8x increase for 50x n — fitted exponent approximately 0.69. The observed exponent below 1.0 reflects that the O(n) selection pass contributes a fixed per-item cost, while the O(n log n) sorting overhead grows sublinearly at these scales. Sorting accounts for the majority of runtime; the greedy selection pass is O(n) and negligible.
...
**Internal validity.** The experimental pipeline is deterministic: instances are generated from seeded PRNGs (seed 42), algorithms execute single-threaded with a global JIT warmup (10,000 iterations per algorithm-family combination) plus 1 per-instance warmup run, and all statistical computations use `random.seed(42)`. This eliminates run-to-run variability as a confounding factor. A 50M node safety cap in the B&B implementation was triggered on Inverse Correlated instances for Branch & Bound at n=1000 (5 instances in fixed mode; 192 instances across multiple families in scaled mode). The 30-second thread-level timeout was never reached (max observed runtime ~9s). Instances hitting the node cap are correctly marked as non-optimal, and the analysis excludes their incomplete node counts from B&B statistics where noted.
